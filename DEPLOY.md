...
### Крок 5: Налаштуй volume mounts (важливо для бекапів)

У `portainer-stack.yml` є bind-mount'и для доступу до файлів хоста з контейнера:

```yaml
volumes:
  - backup-control-data:/app/data
  # Rclone config — змінити лівий шлях на СВІЙ
  - /home/artem/.config/rclone/rclone.conf:/etc/rclone/rclone.conf:ro
  # Директорії зберігання (read-only)
  - /srv/storage:/srv/storage:ro
  - /mnt:/mnt:ro
```

**Що дають ці volumes:**
- `rclone.conf` — rclone всередині контейнера читає твою конфігурацію (Google Drive remote)
- `/srv/storage` та `/mnt` — контейнер бачить файли сервера (для бекапу та disk usage)
- Все `:ro` (read-only) — контейнер не може змінити або видалити твої дані

> Якщо шляхи на твоєму сервері інші, відредагуй їх прямо в Portainer Stack Editor.

### Крок 6: Налаштуй UI

Після деплою відкрий **Settings** у веб-панелі та заповни:
1. **Rclone Configuration** — remote name, шлях до конфігу, папку на GDrive
2. **Disk Usage** — додай mount points (напр. `/srv/storage`, `/mnt/toshiba`) — розміри визначаться автоматично
3. **Telegram** — Bot Token, Chat ID, натисни "Send Test Message" для перевірки
4. **Server Info** — hostname, CPU, RAM, IP адреси (показується на Dashboard)
